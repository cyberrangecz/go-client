...
func (c *Client) GetSandboxDefinition(ctx context.Context, definitionID int64) (*SandboxDefinition, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("%s/sandbox-service/api/v1/definitions/%d", c.Endpoint, definitionID), nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateSandboxDefinition(ctx context.Context, url, rev string) (*SandboxDefinition, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, fmt.Sprintf("%s/sandbox-service/api/v1/definitions", c.Endpoint), strings.NewReader(string(requestBody)))
	if err != nil {
		return nil, err
	}
...
func (c *Client) DeleteSandboxDefinition(ctx context.Context, definitionID int64) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodDelete, fmt.Sprintf("%s/sandbox-service/api/v1/definitions/%d", c.Endpoint, definitionID), nil)
	if err != nil {
		return err
	}
