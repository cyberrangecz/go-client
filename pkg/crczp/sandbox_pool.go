...
func (c *Client) GetSandboxPool(ctx context.Context, poolId int64) (*SandboxPool, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, fmt.Sprintf("%s/sandbox-service/api/v1/pools/%d", c.Endpoint, poolId), nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) CreateSandboxPool(ctx context.Context, definitionId, maxSize int64) (*SandboxPool, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, fmt.Sprintf("%s/sandbox-service/api/v1/pools", c.Endpoint), strings.NewReader(string(requestBody)))
	if err != nil {
		return nil, err
	}
...
func (c *Client) DeleteSandboxPool(ctx context.Context, poolId int64) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodDelete, fmt.Sprintf("%s/sandbox-service/api/v1/pools/%d", c.Endpoint, poolId), nil)
	if err != nil {
		return err
	}
...
func (c *Client) CleanupSandboxPool(ctx context.Context, poolId int64, force bool) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, fmt.Sprintf("%s/sandbox-service/api/v1/pools/%d/cleanup-requests?force=%s",
